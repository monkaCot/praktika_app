...
namespace FieldService.Documents
{
    public static class OrderDocument
    {
        ...
        public static byte[] Generate(ServiceRequest request)
        {
            ...
            var title = new Paragraph("ЗАКАЗ-НАРЯД", titleFont)
            ...
            { SpacingAfter = 5 });
            document.Add(new Paragraph($"Задача: {request.Problem}", normalFont)
            { SpacingAfter = 5 });
            document.Add(new Paragraph($"Статус выполнения: {request.Status}", normalFont)
            { SpacingAfter = 20 });

            document.Add(new Paragraph("Принял: ___________", normalFont)
            { SpacingAfter = 5 });
            document.Add(new Paragraph("Сдал: ___________", normalFont)
            { SpacingAfter = 25 });
